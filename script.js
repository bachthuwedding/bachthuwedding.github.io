(() => {
  ...
  function loadImage(
    ...
    imagePromises.set(image, promise);
    return promise;
  }
  ...
  openingCardButton?.addEventListener(
    "pointerdown",
    () => {
      loadPage(
        pages[0],
  ...
  function saveLuckyNumber(value) {
    if (LUCKY_TEST_MODE) {
      return true;
    }
  ...
  function formatLuckyNumber(value) {
  ...
  function clearLuckyFireworks() {
    ...
    luckyFireworks.replaceChildren();
  }
  ...
  function lockLuckyNumber(
    ...
    if (save) {
      saveLuckyNumber(value);
    }

    showLuckyNumber(value);
  ...
  function updateAttendanceState() {
    if (rsvpAttendanceNo?.checked) {
      rsvpCount = 0;

      if (rsvpGuestCount) {
        rsvpGuestCount.textContent = "0";
      }

      if (rsvpMinus) {
        rsvpMinus.disabled = true;
      }

      if (rsvpPlus) {
        rsvpPlus.disabled = true;
      }
  ...
  rsvpAttendanceYes?.addEventListener(
    "change",
    updateAttendanceState
  );

  rsvpAttendanceNo?.addEventListener(
    "change",
    updateAttendanceState
  );

  rsvpVideo?.addEventListener(
    "change",
    () => {
      const file =
        rsvpVideo.files?.[0];
  ...
  function restoreRsvp() {
    ...
    try {
      ...
      if (data.attendance === "no") {
        if (rsvpAttendanceNo) {
          rsvpAttendanceNo.checked = true;
        }

        if (rsvpAttendanceYes) {
          rsvpAttendanceYes.checked = false;
        }

        rsvpCount = 0;
      } else {
        if (rsvpAttendanceYes) {
          rsvpAttendanceYes.checked = true;
        }

        rsvpCount = Number.isInteger(
          data.count
        )
          ? Math.min(
              10,
